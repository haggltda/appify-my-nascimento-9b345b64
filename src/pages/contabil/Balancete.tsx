...
export default function Balancete() {
  ...
  const q = useQuery({
    queryKey: ["balancete", empresaId, dataIni, dataFim, trigger],
    enabled: !!empresaId,
    queryFn: async () => {
      ...
      return (data ?? []) as Array<{ conta_id: string; classificacao: string; descricao: string; natureza: string; debito: number; credito: number; saldo: number }>;
    },
  });

  const totalD = (q.data ?? []).reduce((s, r) => s + Number(r.debito || 0), 0);
  const totalC = (q.data ?? []).reduce((s, r) => s + Number(r.credito || 0), 0);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="ml-auto text-sm text-muted-foreground">
          Total D: <strong className="text-foreground">{fmt(totalD)}</strong> · Total C: <strong className="text-foreground">{fmt(totalC)}</strong> · Diferença: <strong className={Math.abs(totalD - totalC) < 0.01 ? "text-emerald-500" : "text-red-500"}>{fmt(totalD - totalC)}</strong>
        </div>
      </div>
      ...
      <div className="card-elevated overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {q.isLoading && <tr><td colSpan={6} className="px-3 py-6 text-center text-muted-foreground">Carregando…</td></tr>}
            {(q.data ?? []).map(r => (
              <tr key={r.conta_id} className="border-t border-border/60">
                ...
                <td className="px-3 py-2 text-center text-xs">{r.natureza}</td>
                <td className="px-3 py-2 text-right">{fmt(Number(r.debito))}</td>
                <td className="px-3 py-2 text-right">{fmt(Number(r.credito))}</td>
                <td className={`px-3 py-2 text-right font-semibold ${Number(r.saldo) < 0 ? "text-red-500" : ""}`}>{fmt(Number(r.saldo))}</td>
              </tr>
            ))}
          </tbody>
        </table>
