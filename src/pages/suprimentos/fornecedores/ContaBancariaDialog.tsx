...
export function ContaBancariaDialog({ open, onOpenChange, fornecedorId, empresaId, conta, onSaved }: Props) {
  ...
  const save = async () => {
    ...
    const { error } = conta?.id
      ? await supabase.from("fornecedor_conta_bancaria").update(payload).eq("id", conta.id)
      : await supabase.from("fornecedor_conta_bancaria").insert(payload);
    setSaving(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4 py-2">
          ...
          <div></div>
          <div><Label>Titular (nome)</Label><Input maxLength={200} value={form.titular_nome} onChange={(e) => set("titular_nome", e.target.value)} placeholder="Se diferente do fornecedor" /></div>
