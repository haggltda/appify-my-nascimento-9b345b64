-- Solicitações ERP: 2 ajustes —
-- 1) Avançar de "Aprovações e Priorização" pra "Definição de Responsável" passa a
--    ser exclusivo do Gerente de Sistemas (era Comitê). A definição de prioridade
--    em si continua só com Comitê, não muda.
-- 2) Novo campo "Status de Desenvolvimento" na etapa Desenvolvimento e Ajustes,
--    editável por quem já edita progresso/prazo (Desenvolvedores/Gerente de
--    Sistemas), pra aparecer como selo permanente no card do quadro.

ALTER TABLE public.sistema_solicitacao
  ADD COLUMN IF NOT EXISTS status_desenvolvimento text
    CHECK (status_desenvolvimento IS NULL OR status_desenvolvimento IN ('em_desenvolvimento', 'em_validacao', 'em_correcao', 'finalizado'));
...
BEGIN
  ...
  -- Progresso, prazo e status de desenvolvimento: só Desenvolvedores/Gerente de
  -- Sistemas, só em Desenvolvimento e Ajustes.
  IF NEW.progresso_pct IS DISTINCT FROM OLD.progresso_pct
    ...
    IF OLD.etapa <> 'desenvolvimento_ajustes' OR NOT (public.tem_acesso_menu('sistemas_desenvolvedores') OR public.tem_acesso_menu('sistemas_gerente_sistemas')) THEN
      RAISE EXCEPTION 'Progresso, prazo e status de desenvolvimento só podem ser atualizados na etapa Desenvolvimento e Ajustes, por Desenvolvedores ou Gerente de Sistemas.';
    END IF;
  END IF;
