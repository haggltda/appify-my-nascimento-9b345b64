-- Exige assinatura (aba "Assinaturas") antes de avancar de etapa: cada uma
-- das 13 transicoes de avanco so libera se, na etapa atual, ja existir pelo
-- menos 1 assinatura por CADA papel exigido (logica E entre os papeis).
-- 'criar_solicitacao' e tratado a parte: significa quem criou o card
-- (criado_por), nao a permissao geral 'sistemas_criar_solicitacao'.
-- So afeta "Avancar" -- Retornar/Reprovar/Recusar continuam livres.
...
DECLARE
  ...
  v_faltando text[];
BEGIN
  -- Bypass: usado só pela correção automática de prazo vencido (sistema_corrigir_prazos_vencidos),
  -- que já recalculou o vencimento no servidor antes de chamar este UPDATE — não passa por nenhuma
  -- das checagens de papel abaixo. (A migração de dados em si desliga o trigger inteiro via
  -- DISABLE TRIGGER, não usa este flag.)
  ...
  -- Recusar/Encerrar: liberado em algumas etapas específicas
  ...
  IF NEW.etapa IS DISTINCT FROM OLD.etapa THEN
    ...
    -- Matriz de transição com checagem de papel (Avançar/Retornar/Reprovar):
    v_ok := CASE
    ...
    -- Assinaturas obrigatórias antes de avançar: mapeia cada uma das 13
    -- transições de avanço pros papéis exigidos (lógica E entre eles). NULL
    -- = não é uma transição de avanço gateada (retornar/recusar continuam
    -- livres). 'criar_solicitacao' é tratado à parte: significa quem criou
    -- ESSE card (criado_por), não a permissão geral.
