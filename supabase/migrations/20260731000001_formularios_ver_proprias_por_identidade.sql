...
-- `criado_por = auth.uid()`, mas as respostas chegam pelo LINK PÚBLICO — a
-- coluna criado_por não tem default no banco e ficava sempre nula. Ou seja, a
-- condição nunca batia e a leitura devolvia vazio.
--
-- Correção: "minha resposta" passa a valer quando EU sou o dono da linha
-- (criado_por, para quem enviou logado — agora preenchido no insert) OU quando
-- EU sou o respondente identificado, casando o nome gravado na resposta com o
-- Nome do meu cadastro (EMPREGADOS.auth_user_id = auth.uid()).
--
...
CREATE OR REPLACE FUNCTION public.cs_form_minha_resposta(_criado_por text, _respondente_nome text)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT (_criado_por IS NOT NULL AND _criado_por = auth.uid()::text)
...
$$;
REVOKE EXECUTE ON FUNCTION public.cs_form_minha_resposta(text, text) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.cs_form_minha_resposta(text, text) TO authenticated;
