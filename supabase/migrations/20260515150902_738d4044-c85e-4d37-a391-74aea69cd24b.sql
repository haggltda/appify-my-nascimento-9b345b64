...
CREATE OR REPLACE FUNCTION public.gerar_lancamento_contabil(
  ...
  p_conta_banco_id    uuid DEFAULT NULL,
  p_conta_debito_id   uuid DEFAULT NULL,
  p_conta_credito_id  uuid DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE empresa_id = p_empresa_id AND codigo_evento = p_codigo_evento AND ativo = true
  LIMIT 1;
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Lançamento % não encontrado', p_lancamento_id; END IF;
  IF v_orig.status = 'estornado' THEN RAISE EXCEPTION 'Lançamento % já estornado', p_lancamento_id; END IF;

  v_hash := md5('estorno|'||p_lancamento_id::text);
