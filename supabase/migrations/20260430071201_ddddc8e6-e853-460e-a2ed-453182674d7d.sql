-- Optimized set-based materialization
...
BEGIN
  ...
  -- Build alias map and column metadata
  WITH cols AS (
  ...
  SELECT
    coalesce(jsonb_object_agg(norm, nome_destino), '{}'::jsonb)
    INTO v_alias_map
    FROM all_aliases;
  ...
  -- Reset: validations + staging rows for this batch
  ...
  -- Build a temp table with normalized payloads in a single set-based operation
  ...
  -- Normalize date fields (dd/mm/yyyy -> yyyy-mm-dd) inside payload
  ...
  -- Detect missing required fields → bloqueante
  ...
  -- Build dynamic INSERT for staging using only known columns
  ...
  -- Insert valid rows in a single statement
  v_sql := format(
  ...
  -- Unknown columns (one info entry per unknown header)
  WITH all_keys AS (
