CREATE INDEX IF NOT EXISTS idx_centros_custo_gestor ON public.centros_custo (gestor_user_id, empresa_id) WHERE ativo = true;
...
BEGIN
  ...
  IF v_alvo = 'requisicao_compra' THEN
    ...
    SELECT diretor_user_id INTO v_diretor
    FROM public.empresas WHERE id = v_empresa_id;

    v_responsavel := COALESCE(v_gestor_cc, v_diretor);
    ...
    INSERT INTO public.sup_aprov_etapa (
      ...
      _fluxo_id, v_instancia_id, 1, 'Aprovação de retirada', 'bloqueante', v_responsavel,
      0, 'normal', true
    );
    ...
    IF v_vincula THEN
      ...
      IF NOT v_tem_orcamento THEN
        INSERT INTO public.sup_aprov_etapa (
          ...
          _fluxo_id, v_instancia_id, 2, 'Aprovação por ultrapassar orçamento', 'bloqueante', v_responsavel,
          0, 'urgente', true
        );
      END IF;
    END IF;
