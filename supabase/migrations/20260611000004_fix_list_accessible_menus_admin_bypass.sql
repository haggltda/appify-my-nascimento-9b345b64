-- Corrige list_accessible_menus: admin agora também respeita allow=false
-- em screen_permission_user. Antes, a branch de admin retornava TODOS os menus
-- sem checar overrides individuais, fazendo com que usuários admin nunca pudessem
-- ser bloqueados pelo painel de administração.
...
AS $$
  ...
  -- Resolve o override mais específico por menu (empresa-específico tem prioridade sobre global)
  ...
  resolved AS (
    ...
    -- Antes retornava tudo sem checar overrides, impossibilitando bloquear admins.
    ...
    -- Não-admin: acesso via role/perfil + allow=true explícito, menos allow=false explícito.
    SELECT am.menu_codigo
      FROM active_menus am
      LEFT JOIN override_resolved o ON o.menu_codigo = am.menu_codigo
